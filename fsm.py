...
class TocMachine(GraphMachine):
    ...
    def on_enter_choose(self, event):
        ...
        send_text_message(reply_token, "貓咪總是「喵喵」叫，好想知道牠在說什麼！ (C_C)")
    ...
    def on_enter_hulu(self, event):
        ...
        reply_token = event.reply_token
        send_text_message(reply_token, "是「幼貓」?是「成貓撒嬌」?還是「緊張不舒服」?(*>w<)")

    def on_exit_hulu(self, event):
        print("Leaving hulu")
    ...
    def on_exit_flighty1(self, event):
    ...
    def on_exit_nervous1(self, event):
    ...
    def on_enter_ss(self, event):
        ...
        reply_token = event.reply_token
        send_text_message(reply_token, "是「遇到敵人」還是「處在陌生的環境」呢?(*>w<)")

    def on_exit_ss(self, event):
        print("Leaving ss")
    ...
    def on_enter_y(self, event):
        ...
        reply_token = event.reply_token
        send_text_message(reply_token, "貓感受到威脅程度爆表卻無路可逃，打算用盡力氣與敵人做最後的搏鬥。\n\n通常家貓在接受美容（如洗澡）或醫療的時候，或是貓咪在地盤被入侵，僵持不下、互相對峙的時候，都會發出這樣的叫聲。")

    def on_exit_y(self, event):
        print("Leaving y")
